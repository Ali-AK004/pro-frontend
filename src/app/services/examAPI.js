...
export const examAPI = {
  ...
  exams: {
    ...
    getByLesson: (lessonId) =>
      apiClient.get(`/lessons/${lessonId}/exams`),
  ...
  validation: {
    validateExamData: (examData) => {
      ...
      if (!examData.timeLimitMinutes || examData.timeLimitMinutes < 1) {
        errors.timeLimitMinutes = 'مدة الامتحان يجب أن تكون دقيقة واحدة على الأقل';
      }
      ...
      if (!examData.questions || examData.questions.length === 0) {
        ...
        examData.questions.forEach((question, index) => {
          ...
          if (!question.points || question.points <= 0) {
            errors[`question_${index}_points`] = `درجات السؤال ${index + 1} يجب أن تكون أكبر من صفر`;
          }

          if (!question.answers || question.answers.length === 0) {
            errors[`question_${index}_answers`] = `السؤال ${index + 1} يجب أن يحتوي على إجابة واحدة على الأقل`;
          } else {
            ...
            if (question.questionType === 'SINGLE_CHOICE' || question.questionType === 'TRUE_FALSE') {
              if (correctAnswers.length !== 1) {
                errors[`question_${index}_correct`] = `السؤال ${index + 1} يجب أن يحتوي على إجابة صحيحة واحدة فقط`;
              }
            } else if (question.questionType === 'MULTIPLE_CHOICE') {
              if (correctAnswers.length === 0) {
                errors[`question_${index}_correct`] = `السؤال ${index + 1} يجب أن يحتوي على إجابة صحيحة واحدة على الأقل`;
              }
            }

            question.answers.forEach((answer, answerIndex) => {
              if (!answer.answerText?.trim()) {
                errors[`question_${index}_answer_${answerIndex}`] = `نص الإجابة ${answerIndex + 1} للسؤال ${index + 1} مطلوب`;
              }
            });
          }
        });
